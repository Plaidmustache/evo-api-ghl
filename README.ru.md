# Интеграция [Evolution API](https://evolution-api.com) для [GoHighLevel](https://www.gohighlevel.com)

Данная интеграция обеспечивает взаимодействие с WhatsApp в [GoHighLevel](https://www.gohighlevel.com) (GHL) через
платформу [Evolution API](https://evolution-api.com). Она состоит из
адаптера NestJS, обеспечивающего связь между двумя платформами.

## Требования к самостоятельному размещению

**Важно:** Данная интеграция требует наличия собственного self-hosted инстанса [Evolution API](https://evolution-api.com).
Evolution API — это open-source WhatsApp API, который вы разворачиваете и управляете самостоятельно.

Перед использованием данной интеграции вам необходимо:

1. **Развернуть Evolution API** — Настроить собственный инстанс Evolution API на вашей инфраструктуре
2. **Настроить подключение WhatsApp** — Подключить ваш(и) номер(а) WhatsApp к вашему инстансу Evolution API
3. **Получить учетные данные инстанса** — Получить Instance ID и API Token из панели управления Evolution API

Инструкции по настройке Evolution API доступны в [документации Evolution API](https://doc.evolution-api.com).

## Обзор

Данная документация проведет вас через процесс настройки вашей собственной интеграции
между [Evolution API](https://evolution-api.com) и [GoHighLevel](https://www.gohighlevel.com).
Вы:
...
3. Свяжете один или несколько инстансов [Evolution API](https://evolution-api.com) с вашим
...
- Предоставляет конечные точки для вебхуков от [GoHighLevel](https://www.gohighlevel.com)
  и [Evolution API](https://evolution-api.com)
- Создаёт и управляет контактами из WhatsApp в [GoHighLevel](https://www.gohighlevel.com)
- Поддерживает несколько инстансов [Evolution API](https://evolution-api.com) на один суб-аккаунт с удобным интерфейсом
  управления
...
- Node.js 20 или выше
- Self-hosted инстанс [Evolution API](https://evolution-api.com) с подключённым(и) номером(ами) WhatsApp
...
5. **Сгенерируйте учетные данные:**
    - Перейдите в раздел "Client Keys"
...
6. **Создайте Conversation Provider:**
    ...
    - Имя: "Evolution API" (или любое другое по вашему выбору)
    - Тип: **SMS**
...
7. **Настройте пользовательскую страницу:**
    ...
    - Это предоставит пользователям интерфейс для управления их инстансами Evolution API
...
3. **Настройте переменные окружения:**
   ...
   ```
    ...
    - `GHL_CONVERSATION_PROVIDER_ID`: Из шага 6 настройки приложения GHL
...
2. **Получите доступ к интерфейсу управления инстансами WhatsApp:**
    - После установки вы можете перейти к пользовательской странице для управления вашими инстансами Evolution API (появится
      в боковой панели)
    - Интерфейс позволит вам добавлять/управлять несколькими инстансами

3. **Добавьте инстансы Evolution API:**
    - Используйте интерфейс управления для добавления ваших инстансов Evolution API
    - Для каждого инстанса предоставьте:
        - Имя инстанса (необязательно, для удобной идентификации)
        - ID инстанса (из панели управления Evolution API)
        - API Token (из панели управления Evolution API)
    - Вы можете добавить несколько инстансов и управлять ими независимо

4. **Управляйте вашими инстансами:**
    - Просматривайте все ваши подключенные инстансы Evolution API
...
1. Когда клиент отправляет сообщение на любой из ваших номеров WhatsApp:
    - [Evolution API](https://evolution-api.com) получает сообщение и отправляет его в ваш адаптер
...
1. Чтобы ответить контакту WhatsApp:
    ...
    - Сообщение будет направлено через ваш адаптер к соответствующему инстансу [Evolution API](https://evolution-api.com)
...
**Диалоги с несколькими инстансами:** Если один и тот же номер телефона пишет на несколько ваших инстансов
Evolution API (разные номера WhatsApp), данная интеграция **не будет** создавать отдельные диалоги. Все сообщения от этого
